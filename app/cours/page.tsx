  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setSubjects(withCounts.sort((a, b) => a.name.localeCompare(b.name, "fr")));
      } finally {
        ...
        <section className="section course-list" aria-live="polite">
          ...
          {subjects.map((subject) => (
            <Link href={`/cours/${subject.id}`} className="course-row" key={subject.id}>
              <span className="course-mark" style={subject.color ? { background: subject.color } : undefined}>{subject.name.slice(0, 1)}</span>
              <div><strong>{subject.name}</strong><p>Cours, chapitres et ressources de la matière</p></div>
              <span className="course-count">{subject.chapterCount} chapitre{subject.chapterCount > 1 ? "s" : ""} →</span>
            </Link>
          ))}
        </section>
      </div>

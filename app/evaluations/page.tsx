    ...
    <AppShell>
      <div className="page-content">
        ...
        <p className="page-description">
          Contrôles et évaluations publiés pour ton établissement.
        </p>
        <section className="section exercise-list">
          ...
          {!loading && upcoming.length === 0 && (
            <div className="empty-state">
              ...
              <p>Les contrôles apparaîtront ici dès leur publication.</p>
            </div>
          )}
          {upcoming.map((assessment) => (
            <article className="exercise-row" key={assessment.id}>
              <div>
                <span className="todo-subject">{assessment.subjectId}</span>
                <strong>{assessment.title}</strong>
                <p>{new Date(assessment.date).toLocaleDateString("fr-FR", {
